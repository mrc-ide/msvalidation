...
```{r}
#saveRDS(ms_runs,"./ms_runs.rds")
#saveRDS(ms_values, "./ms_values.rds")
ms_runs <- readRDS("./ms_runs.rds")
ms_values <- readRDS("./ms_values.rds")

# creating a function for running the odin model given an assigned EIR value
get_dmm <- function(eir) {
  run_model(model = 'odin_model',
                     time = period*year,
                     age = c(0, 0.25, 0.5, 0.75, 1, 1.25, 1.5, 1.75, 2, 3, 3.5, 4, 4.5, 5, 7.5, 10, 15, 20, 30, 40, 50, 60, 70, 80),
                     het_brackets = 5,
                     init_EIR = eir,
                     init_ft = 0,
                     eta = eta)
...
get_dmm_values <- function(eq,eir) {
  u5 <- eq_ages < 5 # equilibrium model output gives a matrix of values for each age. for DMM we don't need this
  g2l10 <- (eq_ages > 2) & (eq_ages < 10) # ^ same
  c(
    total_m = NA,
    clinical_incidence = mean(eq$inc) * year, # check and see if DMM outputs daily or yearly CI. probably is daily. taking the mean is correct.
    clinical_incidence_0_5 = mean(eq$inc05) * year,
    prevalence_2_10 = mean(eq$prev), # go back to scripts for run_model() and figure out how they calculated inc05. Ettie Unwin worked on this, can ask her
    eir = eir
  )
}
