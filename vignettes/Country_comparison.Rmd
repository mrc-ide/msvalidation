...
Need to modify malariasimulation so that `n_inc_clinical` includes everyone in the age range who was treated.
...
IM <- params %>% 
  set_bednets(
    ...
    retention = 5 * year,
    dn0 = matrix(rep(.41,15), nrow=15, ncol=1), # or .533
    rn = matrix(rep(.56,15), nrow=15, ncol=1),
    rnm = matrix(rep(.24,15), nrow=15, ncol=1),
    gamman = rep((5*year)/2,15) # or 963.6
  )
...
# actellic: c(2.024412039,  -0.008887407, -2.222632837, 0.008469928, 1.231733302, -0.008887407)
...
IM <- params %>% 
  set_spraying(
    timesteps=seq(5,19,1)*year+warmup,
    coverages=rep(0.8,15),
    ls_theta = matrix(rep(2.025,15), nrow=15, ncol=1),
    ls_gamma = matrix(rep(-0.009,15), nrow=15, ncol=1),
    ks_theta = matrix(rep(-2.222,15), nrow=15, ncol=1),
    ks_gamma = matrix(rep(0.008,15), nrow=15, ncol=1),
    ms_theta = matrix(rep(-1.232,15), nrow=15, ncol=1),
    ms_gamma = matrix(rep(-0.009,15), nrow=15, ncol=1)
  )
...
* there is no way to set MDA as an intervention in `mlgts`.
...
IM <- params %>% 
  set_drugs(list(AL_params, SP_AQ_params, DHA_PQP_params)) %>%
  set_mda(drug=1,time=5*year+warmup,coverage=0.8,min_age=0.5*year,max_age=100*year)
