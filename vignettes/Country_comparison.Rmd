...
```{r, fig.height = 5, fig.width = 7}
year <- 365
warmup <- 5 * year # not used for now
sim_length <- 19 * year
...
run_nmodel <- function(setparams){
  ...
  mutate(t=timestep/365+2000, # assigning year
         ...
         clin_inc_0_1825=case_when(!is.na(n_inc_clinical_0_1825)~n_inc_clinical_0_1825/n_0_1825,
                                   is.na(n_inc_clinical_0_1825)~0),
...
IM <- params %>% 
  set_spraying(
    timesteps=seq(5,19,1)*year,
    coverages=rep(0.8,15),
    ls_theta = matrix(2.025, nrow=1, ncol=1),
    ls_gamma = matrix(-0.009, nrow=1, ncol=1),
    ks_theta = matrix(-2.222, nrow=1, ncol=1),
    ks_gamma = matrix(0.008, nrow=1, ncol=1),
    ms_theta = matrix(-1.232, nrow=1, ncol=1),
    ms_gamma = matrix(-0.009, nrow=1, ncol=1)
  )
...
IM <- params %>% 
   set_drugs(list(AL_params, SP_AQ_params, DHA_PQP_params)) %>%
   set_smc(drug=1,timesteps=seq(5,19,1)*year,coverages=rep(0.8,15),min_age=0,max_age=100)
